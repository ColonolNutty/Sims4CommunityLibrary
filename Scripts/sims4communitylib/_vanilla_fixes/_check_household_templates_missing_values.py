...
def _common_give_details_when_templates_missing(original, cls, sim_creator) -> Any:
    try:
        if cls._household_templates_override and None in cls._household_templates_override:
            ...
            for household_template_override in cls._household_templates_override:
                if household_template_override is not None:
                    household_template_overrides.append(household_template_override)
            cls._household_templates_override = tuple(household_template_overrides)
        original_result = original(sim_creator)
    except Exception as ex:
        log.format_error_with_message('Error occurred trying to get templates.', me=cls, _household_templates_override=cls._household_templates_override, exception=ex)
        raise ex
