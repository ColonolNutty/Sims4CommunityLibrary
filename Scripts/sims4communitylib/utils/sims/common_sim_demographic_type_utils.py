...
class CommonSimDemographicTypeUtils(_HasS4CLClassLog):
    ...
    def is_sim_contained_in_demographic_flags(cls, sim_info: SimInfo, demographic_flags: 'CommonSimDemographicType') -> bool:
        ...
        if CommonBitwiseUtils.contains_any_flags(demographic_flags, CommonSimDemographicType.get_all_flags(exclude_values=(CommonSimDemographicType.CONTROLLED, CommonSimDemographicType.HOUSEHOLD, CommonSimDemographicType.NON_HOUSEHOLD))):
        ...
        if CommonBitwiseUtils.contains_all_flags(demographic_flags, CommonSimDemographicType.NON_HOUSEHOLD) and CommonSimTypeUtils.is_non_player_sim(sim_info):
            ...
            return True
        cls.get_log().format_with_message('Sim failed all flags', sim=sim_info, demographic_flags=demographic_flags, sim_demographic_flags=sim_demographic_flags)
        return False
    ...
    def is_sim_contained_in_demographics(cls, sim_info: SimInfo, demographics: Tuple['CommonSimDemographicType']) -> bool:
        ...
        has_sim_specific_flags = False
        for demographic in demographics:
            if demographic not in (CommonSimDemographicType.CONTROLLED, CommonSimDemographicType.HOUSEHOLD, CommonSimDemographicType.NON_HOUSEHOLD):
                has_sim_specific_flags = True
                break

        if has_sim_specific_flags:
            ...
            for sim_demographic in sim_demographics:
                if sim_demographic not in demographics:
                    cls.get_log().format_with_message('Sim failed top flags', sim=sim_info, demographics=demographics, sim_demographics=sim_demographics)
                    return False
